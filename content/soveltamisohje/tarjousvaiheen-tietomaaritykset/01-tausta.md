...
IFC-tietomallistandardissa rakenneosien tunnistamiseksi objekteille voidaan määrittää IFC-luokkia, kuten esimerkiksi _ifcwall_. Useilla IFC-luokilla on lisäksi _PredefinedType_-ominaisuus, joka tarkentaa luokan tyyppiä. Esimerkiksi _ifcwall_-luokka voi sisältää _PredefinedType_-arvoja, kuten `ELEMENTEDWALL` tai `SOLIDWALL`. Standardissa määriteltyjen luokitteluiden tarkkuustaso riittää karkeaan luokitteluun, mutta ei mahdollista riittävän tarkkaa tunnistamista määrä- ja kustannuslaskentaa varten. Tämän vuoksi tarvitaan lisäattribuutteja, jotta tietomallin avulla saavutetaan vaadittava tarkkuustaso. Ohjeessa kuvattu ratkaisusta on tehty myös esimerkki malli Tekla Structures-ohjelmassa ja mallista IFC-tiedosto, eli ratkaisu on myös toteuttamiskelpoinen nykyisillä ohjelmistoilla.
...
- Muut tietomallin tietokentät ja sallitut arvot
